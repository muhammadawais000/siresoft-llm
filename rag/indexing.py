"""Persist split, embedded chunks to Postgres."""
...
def persist_chunks(document: Document, chunk_dicts: list[dict], vectors: list[list[float]]) -> int:
    ...
    # diffed/updated in place — this keeps chunk_index/embedding/content
    # always mutually consistent, and re-indexing is expected to be rare
    # enough that the extra writes don't matter.
    ...
    objs = [
        ...
        for c, vector in zip(chunk_dicts, vectors)
    ]
